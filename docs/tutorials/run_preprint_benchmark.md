...
>   ```
> - **Everyone must `git pull` and purge caches** (§1) — the fixes are in
>   `6060ef3`; the tutorial you were given at `a21c54b` predates all of them.
>
...
### Purge stale caches — required once, before your first run

The manifests were rebuilt on 2026-07-21 (survival fix). Prep caches the task
CSVs and splits, and it can detect a *schema* change but **not** that a
same-schema CSV is merely out of date — so a stale cache would silently feed old
labels into training. Clear yours once:

```bash
BD=/project/rrg-jma/shared/Pathology/autoMIL/phase2/$DS/benchmark_5fold
rm -rf $BD/dataset_csv $BD/splits          # regenerated by step (a); features/ is NOT touched
```

Only `dataset_csv/` and `splits/` — leave `features/`, `results/` alone (those
are hundreds of GB of converted features). Redo this after any manifest rebuild.
