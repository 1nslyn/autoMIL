...
runbook). The campaign protocol is now `preprint-v4`: **30 charged attempts
...
| B1 | P1 | C2 | **The Ladder margin's noise floor is self-reported.** `primary_se` is read verbatim off `result.json` (`terminal_writer.py:283`); `scoring.cross_fold_se` is never called in `src/automil/`; `cells/reconcile.aggregate_folds` has fold primary values in hand (`cells/reconcile.py:52-95`) and computes no SE, so budget-killed/partial nodes silently drop to the bare δ. The same machinery that refuses to trust the reported `primary_value` (CR-1b) trusts the reported SE that *gates* it. | Recompute SE at ingest from `result["validation_folds"]` (agent-visible, val-only, already emitted — `evaluate.py:213`); prefer recomputed, log disagreement — the exact CR-1b pattern. Same helper in `aggregate_folds`. |
| B2 | P1 | C5 | **`scoring.formula` fails open on a typo and the template teaches the failure.** `config.yaml.j2:148-155` says "documentation-only … NOT evaluate[d]" with examples (`"accuracy"`, `"(val_auc + val_bacc) / 2"`) that `scoring.py:59-66` rejects as reducer names; `terminal_writer.py:229-232` catches the ValueError and **trusts the reported primary_value** — CR-1b silently disabled by following the template's own comment. | Rewrite the template comment (reducer semantics: `mean`/`max`/`min`/`trust_reported`); validate the reducer name at graph seeding — **the raise must sit outside the blanket `except Exception` at `graph.py:258-260`** — and in `automil check`; fix the `scoring.py:56-58` docstring. (Reassurance from review: the materialized campaign configs never set `scoring.formula`, so the campaign runs CR-1b on the `mean` default and reproduces both declared primary values exactly — this fix protects operators, not the campaign.) |
...
10. **B1 + B6 (one unit)** — shared ingest helper (key-guard + primary_value
...
4. **Self-reported inputs to enforcement**: primary_value was fixed (CR-1b), SE was
...
5. **What is already right** (and the paper should say so): born-sealing at the
   ...
   the `AUTOMIL_CERTIFY` print gate inside the vendored trainer. (The once-informational
   `metrics.composite_formula` key has since been removed outright: the
   declaration pair is `scoring.formula` + `metrics.primary`, and the
   selection value is the declared primary metric itself.)
