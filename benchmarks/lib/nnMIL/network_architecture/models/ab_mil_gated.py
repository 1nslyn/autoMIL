"""Gated-attention ABMIL — faithful port of the original (Ilse et al., 2018).
...
implementation, ``GatedAttention`` in
``benchmarks/lib/AttentionDeepMIL/model.py`` (lines ~72-128).

Two adaptations from the reference, both confined to the ends of the network;
the gated-attention core in the middle is verbatim:
...
2. The reference's binary ``Sigmoid`` head is replaced by a
   ``Linear(M -> num_classes)`` head emitting K-class logits (no sigmoid),
   so this model trains with cross-entropy and reports through the same
   metrics path as the rest of the nnMIL zoo instead of a bespoke bernoulli
   objective.

The attention gating itself (``attention_V``, ``attention_U``,
``attention_w``, softmax over instances, weighted sum) matches the reference
exactly, including the paper's hidden dimensions M=500, L=128.
"""
...
class AB_MIL_Gated(nn.Module):
    """Gated-attention MIL over precomputed instance features.

    Matches the nnMIL model I/O contract: ``forward`` takes batched input
    ...
    def __init__(self, in_dim=1024, M=500, L=128, num_classes=2, dropout=0.0, K=1):
        ...
        if K != 1:
            raise ValueError(
                f"AB_MIL_Gated only supports a single attention branch (K=1); got K={K}. "
