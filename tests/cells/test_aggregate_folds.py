...
def test_all_folds_completed_returns_completed_status(tmp_path: Path) -> None:
    """K=5 folds present → status='completed', partial_folds==5, correct primary_value mean."""
    primary_values = [0.80, 0.82, 0.84, 0.86, 0.88]
    for i, c in enumerate(primary_values):
        _write_fold(tmp_path, i, primary_value=c)
    ...
    assert result["primary_value"] == pytest.approx(0.84, rel=1e-6)
    # mean of per-fold val_auc (same as primary_value in default fixture)
...
def test_partial_folds_returns_partial_status(tmp_path: Path) -> None:
    """3 of 5 folds → status='partial'; primary_value is mean of 3, NOT zero, NOT NaN."""
    ...
    assert result["expected_folds"] == 5
    # CRITICAL: primary_value must not be zero or NaN
    assert result["primary_value"] == pytest.approx(0.82, rel=1e-6)
    assert result["primary_value"] != 0.0
    assert result["primary_value"] == result["primary_value"]  # not NaN


def test_zero_folds_returns_crashed_status(tmp_path: Path) -> None:
    """Empty directory → status='crash', primary_value=0.0, partial_folds=0.
...
def test_single_fold_returns_partial(tmp_path: Path) -> None:
    """1 of 5 folds → status='partial', partial_folds=1, primary_value=0.75."""
    _write_fold(tmp_path, 0, primary_value=0.75)

    result = aggregate_folds(tmp_path, expected_fold_count=5)

    assert result["status"] == "partial"
    assert result["partial_folds"] == 1
    assert result["primary_value"] == pytest.approx(0.75, rel=1e-6)
...
def test_malformed_fold_skipped_with_warning(tmp_path: Path, caplog: pytest.LogCaptureFixture) -> None:
    ...
    for i in range(5):
        _write_fold(tmp_path, i, primary_value=0.80)
...
def test_metrics_mean_across_folds(tmp_path: Path) -> None:
    ...
    _write_fold(tmp_path, 0, primary_value=0.80, metrics={"val_auc": 0.80, "test_auc": 0.85})
    _write_fold(tmp_path, 1, primary_value=0.82, metrics={"val_auc": 0.82, "test_auc": 0.86})
    _write_fold(tmp_path, 2, primary_value=0.84, metrics={"val_auc": 0.84, "test_auc": 0.87})
...
def test_elapsed_seconds_summed_peak_vram_max(tmp_path: Path) -> None:
    ...
    _write_fold(tmp_path, 0, primary_value=0.80, elapsed=100, peak_vram=4000)
    _write_fold(tmp_path, 1, primary_value=0.80, elapsed=200, peak_vram=4500)
    _write_fold(tmp_path, 2, primary_value=0.80, elapsed=300, peak_vram=4200)
...
def test_null_primary_value_fold_is_skipped_not_counted_as_zero(tmp_path: Path) -> None:
    """A fold whose primary_value was unestimable serializes as ``null`` (never NaN).
    ...
    """
    _write_fold(tmp_path, 0, primary_value=0.80)
    _write_fold(tmp_path, 1, primary_value=0.84)
...
def test_non_finite_primary_value_fold_is_skipped(tmp_path: Path) -> None:
    ...
    (tmp_path / "fold_1_result.json").write_text(
        ...
        '"metrics": {}, "primary_value": NaN, "elapsed_seconds": 10, "peak_vram_mb": 1}'
    )

    result = aggregate_folds(tmp_path, expected_fold_count=5)

    assert result["partial_folds"] == 1
    assert result["primary_value"] == pytest.approx(0.80, rel=1e-6)


def test_null_primary_value_fold_contributes_no_metrics_either(tmp_path: Path) -> None:
    """`primary_value` and `metrics` must describe the SAME fold set (review finding).

    Keeping a dropped fold's surviving metric left `primary_value` averaged over N
    folds and `metrics` over N+1. CR-1b recomputes the primary_value from `metrics`,
    so the two disagreed past PRIMARY_VALUE_TOLERANCE — which fires terminal_writer's
    ...
    for i in range(3):
        _write_fold(tmp_path, i, primary_value=0.75,
    ...
    (tmp_path / "fold_3_result.json").write_text(json.dumps({
        ...
        "primary_value": None, "elapsed_seconds": 40, "peak_vram_mb": 100,
    }))
    ...
    recomputed = recompute_primary_value(result["metrics"])
...
def test_a_key_missing_from_any_fold_is_dropped_not_averaged_unevenly(tmp_path: Path) -> None:
    """`primary_value` and every reported mean must share ONE denominator.

    Dropping a fold whole when its COMPOSITE is null was not enough: a fold can
    carry a finite primary_value and still be missing an individual metric, since
    _write_fold_result_json nulls each independently and the training-script
    contract makes `metrics` keys optional while `primary_value` is required.
    ...
    for i in range(2):
        (tmp_path / f"fold_{i}_result.json").write_text(json.dumps({
            ...
            "primary_value": 0.75, "elapsed_seconds": 100, "peak_vram_mb": 4000,
        }))
    # Finite primary_value, but this fold's test split missed a class.
    (tmp_path / "fold_2_result.json").write_text(json.dumps({
    ...
    # The fold with the null test_auc contributes nothing at all, so primary_value
    ...
    assert not primary_value_disagrees(
        result["primary_value"], recompute_primary_value(result["metrics"]),
    )
...
def test_val_side_uneven_coverage_cannot_trip_the_firewall_alarm(tmp_path: Path) -> None:
    ...
    for i, (comp, auc, bacc) in enumerate(specs):
        (tmp_path / f"fold_{i}_result.json").write_text(json.dumps({
            ...
            "primary_value": comp, "elapsed_seconds": 10, "peak_vram_mb": 100,
        }))
    ...
    recomputed = recompute_primary_value(result["metrics"])
    assert recomputed is None or not primary_value_disagrees(result["primary_value"], recomputed)


def test_validation_folds_carry_primary_values_and_prediction_hashes(tmp_path: Path) -> None:
    """The recovery path must emit the same per-fold evidence contract as a
    normal completion: fold primary values feed the paired keep-margin and
    ``val_predictions_sha256`` feeds no-op detection; without them a recovered
    node silently reverts to the marginal-SE basis."""
    _write_fold(tmp_path, 0, primary_value=0.80)
    _write_fold(tmp_path, 1, primary_value=0.60)
    ...
    assert [e["primary_value"] for e in entries] == [0.80, 0.60]
...
def test_unresolvable_fold_index_drops_the_whole_fold(tmp_path: Path) -> None:
    ...
    not count toward primary_value/primary_se/partial_folds either — a split
