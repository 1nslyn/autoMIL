"""CR-1a (audit 2026-07-23): non-finite primary_value/metric values must never
influence val-firewall selection, and graph.json must stay standards-valid JSON.

Covers three coordinated guards:
- ``validate_result`` rejects non-finite ``primary_value`` and metric values.
...
def _valid_payload(primary_value=0.83):
    return {
        "status": "completed",
...
def test_validate_result_rejects_nonfinite_primary_value(bad):
    payload = _valid_payload(primary_value=bad)
...
def test_collect_result_degrades_nonfinite_token_to_crash(tmp_path, token):
    ...
    (worktree / "result.json").write_text(
        '{"status": "completed", "primary_value": %s, "metrics": {}}' % token
    )
...
def test_collect_result_parses_finite_normally(tmp_path):
    ...
    archive.mkdir(parents=True)
    (worktree / "result.json").write_text(
        json.dumps({"status": "completed", "primary_value": 0.7, "metrics": {"val_auc": 0.7}})
    )
...
def test_graph_save_raises_on_nonfinite(tmp_path):
    ...
    g.meta["best_primary_value"] = math.inf
