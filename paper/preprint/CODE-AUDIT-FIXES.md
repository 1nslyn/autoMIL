...
| H-3 | ABMIL/DTFD silently ignore hyperparameter overrides + **5 inconsistent hyperparameter provenances** | HIGH | ◐ | `hparams.py`(new) | **Step 1 landed:** single declared source of truth (`ARM_HPARAMS`) with per-arm provenance + ONE override path (`resolve_arm_hparams`, None-safe so unset CLI flags never flatten arms onto one schedule). 20 tests incl. a **freeze-guard** pinning every arm's live values → adopting this changes NO running experiment. **Audit finding it exposes:** CLAM runs at **2x its upstream lr** (2e-4 vs CLAM main.py:74 = 1e-4); ABMIL uses Ilse architecture but the shared schedule; TITAN had MIXED provenance; nnMIL hardcodes lr per task type (3e-4 cls / 1e-4 surv) AND 100 epochs vs everyone's 200. Only DTFD is paper-exact. **Step 2 (pending): wire the runners to call the resolver** so overrides actually reach ABMIL/DTFD. **Step 3 = Leo's call:** which arms return to upstream values (that WOULD invalidate the dispatched grid). |
